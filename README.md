...
| Viewport and layer outputs | Native vertex built-ins and converted `SV_ViewportArrayIndex`/`SV_RenderTargetArrayIndex` select viewports and render-target array layers independently of multiview amplification. |
| Mesh, geometry and tessellation | Apple9 or newer. NRI includes indirect drawing in these capabilities; Apple's indirect mesh arguments require Apple9 even though direct mesh drawing starts at Apple7. HLSL geometry/tessellation stages use Converter's mesh/object emulation. Metal 4 has no native patch-draw commands or traditional geometry stage. |
| Vertex input | Dynamic vertex-buffer strides, including per-instance streams and partial rebinding. |
| Draw parameters and draw index | HLSL uses `ENABLE_DRAW_PARAMETERS_EMULATION` / `ENABLE_DRAW_INDEX_EMULATION` and the matching `NRI.hlsl` defines. Target SM6.7 or earlier when using draw-base macros: Converter does not implement the SM6.8 base intrinsics. Indirect draws with parameter emulation use `DrawBaseDesc` / `DrawIndexedBaseDesc`. Direct draw index is zero; indirect index is the original command index. Native shaders use Metal's base/ID built-ins as described in [NRI.metal](Include/NRI.metal). |
| Mutable descriptors | Mutable ranges use directly indexed resource heaps, following NRI's heap convention. Typed descriptor tables retain their declared types. |
...
| Sampler bias and borders | Converter applies sampler LOD bias. Before Apple10, native implicit-LOD sampling passes `NriGetSamplerMipBias` as `metal::bias`; Apple10 applies it in the sampler and the helper returns zero. Border addressing accepts exact transparent black, opaque black and opaque white; other colors return `UNSUPPORTED`. |
| Input attachments | Disabled. Converter's color framebuffer fetch does not establish the full NRI attachment/resource contract, including depth/stencil inputs. Ordinary sampled textures remain available. |
| Other shader restrictions | Native HLSL I8, bottom-left viewport origin, ray position fetch and shader clock remain disabled. Apple8's macOS-only unsigned 64-bit atomic min/max subset does not enable the general I64 atomic capability; that requires Apple9. Clip/cull and mesh-output-component limits remain zero where an exact supported count has not been established. |
